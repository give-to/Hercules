...
if __name__ == "__main__":
    iterator_count = sys.argv[1]
    src_path = sys.argv[2]
    with open(src_path+".bak", 'r') as src_file:
        src_str = src_file.readlines()
        patch = sys.argv[3]
        line_num = int(sys.argv[4])
        dst_str = applyPatch(src_str, patch, line_num)
        dst_str = toFixedLineSrc(dst_str)
    with open(src_path, 'w') as dst_file:
